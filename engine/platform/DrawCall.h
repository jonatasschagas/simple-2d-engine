...
struct Vertex {
  Vertex(Vector2 _position, Vector2 _textureCoordinates, Color _color)
      : position(_position),
        ...
        color(_color) {}
  Vertex(Vector2 _position, Color _color)
...
inline TextureSettings makeTextureSettings(string const& textureName, int tx,
...
inline ColorReplacement makeColorReplacement(bool colorFilter, Color key,
                                             Color replacement,
                                             float threshold) {
...
inline SpriteProperties makeSpriteProperties(float x, float y, float w, float h,
                                             float rotation = 0.0f) {
  SpriteProperties spriteProperties;
...
inline Settings makeSettings(bool flipHorizontal = false,
                             bool topToDown = false, bool scale = true,
                             bool tileMap = false, bool font = false) {
  Settings settings;
