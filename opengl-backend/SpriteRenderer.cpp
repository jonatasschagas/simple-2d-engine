...
void SpriteRenderer::draw(Shader& rShader, glm::mat4 const& rTransform, Texture2D const& rTexture, glm::vec4 const& rTextureCoordinates) {
  rShader.use();
  ...
  string textureName = stringFormat("%d-(%.0f,%.0f,%.0f,%.0f)", rTexture.getId(), rTextureCoordinates.x, rTextureCoordinates.y, rTextureCoordinates.z, rTextureCoordinates.w);
  if (m_textureVAOs.find(textureName) == m_textureVAOs.end()) {
    createTextureVAO(textureName, glm::vec2(rTexture.getWidth(), rTexture.getHeight()), rTextureCoordinates);
  }
