...
bool LTexture::loadFromFile(SDL_Renderer* pRenderer, std::string path) {
  ...
  if (pLoadedSurface == NULL) {
    ...
    // Color key image
    SDL_SetColorKey(pLoadedSurface, SDL_TRUE,
                    SDL_MapRGB(pLoadedSurface->format, 0, 0xFF, 0xFF));
...
void LTexture::render(SDL_Renderer* pRenderer, float x, float y,
                      SDL_Rect* pClip, float w, float h, float angle,
                      bool flipHorizontal) {
  ...
  SDL_RenderCopyExF(pRenderer, m_pTexture, pClip, &renderQuad, angle, NULL,
                    flip);
}
...
SDL_Texture* LTexture::getTexture() { return m_pTexture; }